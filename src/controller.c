...
#if 1
	ret = connectivity_notify_int(ad->resource_info, "Motion", value);
...
static bool service_app_create(void *data)
{
	...
	/*
	 * Preprocess to connect with device(Sensor)
	 */
...
#if 1
	ret = connectivity_set_protocol(CONNECTIVITY_PROTOCOL_HTTP);
	if (ret != 0) _E("Cannot set connectivity type");
#endif

#if 1
	ret = connectivity_set_resource("/door/5", "org.tizen.door", &ad->resource_info);
	 ...
	 */
#if 1
	ad->getter_timer = ecore_timer_add(2.0f , _control_sensors_cb, ad);
	if (!ad->getter_timer) {
...
static void service_app_terminate(void *data)
{
	 ...
	 * Access only when modifying internal functions.
	 */
	controller_fini_internal_functions(); // from controller_internal.c
